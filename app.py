...
def get_mongodb_connection():
    ...
    try:
        ...
        DB_NAME = os.getenv("DB_NAME", "rag_chatbot_db")
...
1. Customer identifier (name, ID, email) - if present
...
INVOICE: [number or NOT_FOUND]"""
    ...
    for line in lines:
        if line.startswith("CUSTOMER:"):
            customer_id = line.replace("CUSTOMER:", "").strip()
        elif line.startswith("INVOICE:"):
            invoice_num = line.replace("INVOICE:", "").strip()
    
    # Search by invoice
    if invoice_num != "NOT_FOUND":
...
Our team will respond within 1-2 hours.

📞 1-800-SUPPORT | 📧 support@company.com"""

# --- Streamlit UI ---
st.title("🤖 Enhanced RAG Chatbot with Embedding Classification")
st.markdown("---")
        ...
        with st.chat_message("assistant"):
            with st.spinner("Processing..."):
                try:
                    ...
                    with st.expander("🎯 Intent Classification Details"):
                    ...
                    # Token count
                    tokens = st.session_state.token_counter.count_tokens(answer)
                    st.caption(f"📊 Tokens: {tokens}/1500")
